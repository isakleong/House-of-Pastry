...
$excel->getProperties()->setCreator('House Of Pastry')
             ...
             ->setTitle("Data Icing")
             ->setSubject("Icing")
             ->setDescription("Data Icing")
             ->setKeywords("Data Icing");
...
$excel->setActiveSheetIndex(0)->setCellValue('A1', "DATA ICING"); // Set kolom A1 dengan tulisan "DATA SISWA"
...
$ambil = $koneksi->query("SELECT * FROM icing");
...
while($data = $ambil->fetch_assoc()){ // Ambil semua data dari hasil eksekusi $sql
  ...
  $excel->setActiveSheetIndex(0)->setCellValue('B'.$numrow, $data['nama_icing']);
  $excel->setActiveSheetIndex(0)->setCellValueExplicit('C'.$numrow, number_format($data['harga_icing']));
...
$excel->getActiveSheet(0)->setTitle("Data Icing");
$excel->setActiveSheetIndex(0);

// Proses file excel
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment; filename="Data Icing.xlsx"'); // Set nama file excel nya
