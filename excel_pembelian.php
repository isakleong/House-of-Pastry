...
$excel->setActiveSheetIndex(0)->setCellValue('C3', "ONGKIR"); // Set kolom C3 dengan tulisan "ONGKIR"
$excel->setActiveSheetIndex(0)->setCellValue('D3', "TANGGAL"); // Set kolom D3 dengan tulisan "TANGGAL"
$excel->setActiveSheetIndex(0)->setCellValue('E3', "TOTAL"); // Set kolom D3 dengan tulisan "Total"
$excel->setActiveSheetIndex(0)->setCellValue('F3', "NOTES"); // Set kolom D3 dengan tulisan "Total"
...
$ambil = $koneksi->query("SELECT * FROM pembelian join pelanggan join ongkir on pembelian.id_pelanggan=pelanggan.id_pelanggan and pembelian.id_ongkir=ongkir.id_ongkir order by pembelian.tanggal_pembelian");
...
while($data = $ambil->fetch_assoc()){ // Ambil semua data dari hasil eksekusi $sql
  ...
  $excel->setActiveSheetIndex(0)->setCellValue('C'.$numrow, $data['nama_kota']);
  $excel->setActiveSheetIndex(0)->setCellValue('D'.$numrow, $data['tanggal_pembelian']);
  $excel->setActiveSheetIndex(0)->setCellValue('E'.$numrow, number_format($data['total_pembelian']));
   $excel->setActiveSheetIndex(0)->setCellValue('F'.$numrow, $data['alamat_tujuan']);
...
$excel->getActiveSheet(0)->setTitle("Data Pembelian");
$excel->setActiveSheetIndex(0);

// Proses file excel
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment; filename="Data Pembelian.xlsx"'); // Set nama file excel nya
