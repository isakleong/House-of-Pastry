...
$excel->getProperties()->setCreator('House Of Pastry')
             ...
             ->setTitle("Data Flavor")
             ->setSubject("Flavor")
             ->setDescription("Data Flavor")
             ->setKeywords("Data Flavor");
...
$excel->setActiveSheetIndex(0)->setCellValue('A1', "DATA FLAVOR"); // Set kolom A1 dengan tulisan "DATA SISWA"
...
$ambil = $koneksi->query("SELECT * FROM flavor");
...
while($data = $ambil->fetch_assoc()){ // Ambil semua data dari hasil eksekusi $sql
  ...
  $excel->setActiveSheetIndex(0)->setCellValue('B'.$numrow, $data['nama_flavor']);
  $excel->setActiveSheetIndex(0)->setCellValueExplicit('C'.$numrow, number_format($data['harga_flavor']));
...
$excel->getActiveSheet(0)->setTitle("Data Flavor");
$excel->setActiveSheetIndex(0);

// Proses file excel
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment; filename="Data Flavor.xlsx"'); // Set nama file excel nya
