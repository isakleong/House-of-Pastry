...
$excel->getProperties()->setCreator('House Of Pastry')
             ...
             ->setTitle("Data Size")
             ->setSubject("Size")
             ->setDescription("Data Size")
             ->setKeywords("Data Size");
...
$excel->setActiveSheetIndex(0)->setCellValue('A1', "DATA SIZE"); // Set kolom A1 dengan tulisan "DATA SISWA"
...
$ambil = $koneksi->query("SELECT * FROM size");
...
while($data = $ambil->fetch_assoc()){ // Ambil semua data dari hasil eksekusi $sql
  ...
  $excel->setActiveSheetIndex(0)->setCellValue('B'.$numrow, $data['nama_size']);
  $excel->setActiveSheetIndex(0)->setCellValueExplicit('C'.$numrow, number_format($data['harga_size']));
...
header('Content-Disposition: attachment; filename="Data Size.xlsx"'); // Set nama file excel nya
